...
- Seedream默认输出 `1328x1776` 竖版海报，共2,358,528像素，低于236万像素成本边界。设置页只接受接近3:4、竖版且少于236万像素的明确宽高值，不接受 `2K` 等不确定像素档位。
- 海报请求最多输入3张原始关键帧，关闭水印，不发送仅Seedream 4.0支持的组图和流式参数。当前成本受控模式不会把参考形象照片作为额外第4张输入；保存的文字形象描述可作为非生物特征风格提示。
