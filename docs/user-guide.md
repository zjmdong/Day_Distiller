...
4. 可在“参考形象”页上传一张清晰的单人正面或半身照片，再填写20–100字的形象描述并保存。当前单海报成本模式只把文字描述加入提示，不把参考照片作为额外第4张输入图；照片保留在电脑本地，为后续可选身份一致性模式准备。
...
6. 等待导入、预处理、全天筛选、单张海报生成、排版和邮件发送完成。系统只选2–3个最有意义的场景，生成一张无文字3:4竖版海报。
